...
Stream_Handshake_IO::get_next_record(bool)
{
	if (m_queue.length >= 4)
	{
		...
		if (m_queue.length >= length + 4)
		{
			...
			Vector!ubyte contents(m_queue.begin() + 4,
...
Vector!ubyte
Stream_Handshake_IO::format(in Vector!ubyte msg,
							Handshake_Type type) const
{
	Vector!ubyte send_buf(4 + msg.length);
...
Vector!ubyte Stream_Handshake_IO::send(in Handshake_Message msg)
{
...
Vector!ubyte
Datagram_Handshake_IO::format_fragment(in ubyte* fragment,
													size_t frag_len,
													ushort frag_offset,
													ushort msg_len,
													Handshake_Type type,
...
{
	Vector!ubyte send_buf(12 + frag_len);
...
Vector!ubyte
Datagram_Handshake_IO::format_w_seq(in Vector!ubyte msg,
												Handshake_Type type,
												ushort msg_sequence) const
{
...
Vector!ubyte
Datagram_Handshake_IO::format(in Vector!ubyte msg,
										Handshake_Type type) const
{
...
Datagram_Handshake_IO::send(in Handshake_Message msg)
{
	...
	std::tuple<ushort, ubyte, Vector!ubyte> msg_info(epoch, msg_type, msg_bits);
