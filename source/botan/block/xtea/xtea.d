...
* XTEA Encryption
*/
void XTEA::encrypt_n(in byte[] input, ref byte[] output) const
{
	while(blocks >= 4)
	{
		xtea_encrypt_4(input, out, &(this->EK[0]));
		in += 4 * BLOCK_SIZE;
		out += 4 * BLOCK_SIZE;
		blocks -= 4;
	}
...
* XTEA Decryption
*/
void XTEA::decrypt_n(in byte[] input, ref byte[] output) const
{
	while(blocks >= 4)
	{
		xtea_decrypt_4(input, out, &(this->EK[0]));
		in += 4 * BLOCK_SIZE;
		out += 4 * BLOCK_SIZE;
		blocks -= 4;
	}
