...
size_t testHashLargerThanN(RandomNumberGenerator rng)
{
    ...
    size_t fails = 0;
    auto priv_key = scoped!ECDSAPrivateKey(rng, dom_pars);
...
size_t testSignThenVer(RandomNumberGenerator rng)
{
    ...
    auto ecdsa = scoped!ECDSAPrivateKey(rng, dom_pars);
...
size_t testCreateAndVerify(RandomNumberGenerator rng)
{
    ...
    ECGroup dom_params = ECGroup(curve, p_G, bi_order_g, BigInt(1));
    if (!p_G.onTheCurve())
