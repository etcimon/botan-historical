...
uint check_against_copy(const Private_Key orig,
...
size_t test_x509()
{
	...
	X509_Certificate ca_cert = X509::create_self_signed_cert(ca_opts(), ca_key, hash_fn, rng);
	/* Create user #1's key and cert request */
	DSA_PrivateKey user1_key(rng, DL_Group("dsa/botan/2048"));

	PKCS10_Request user1_req = X509::create_cert_req(req_opts1(), user1_key, "SHA-1", rng);
	...
	PKCS10_Request user2_req = X509::create_cert_req(req_opts2(), user2_key, hash_fn, rng);
	...
	{
		writeln("FAILED: User cert #1 was not revoked - "
	...
	{
		writeln("FAILED: User cert #2 was not revoked - "
					 << result_u2.result_string());
		++fails;
	}
	...
	if(!result_u1.successful_validation())
	{
		writeln("FAILED: User cert #1 was not un-revoked - "
					 << result_u1.result_string());
		++fails;
	}
